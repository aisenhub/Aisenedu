...
export function PrintPreview({ appearance, maxLines, onScaleChange, pages, scale }: PrintPreviewProps) {
  ...
  return (
    <section aria-labelledby="preview-heading" className="rounded-2xl border border-border bg-surface-muted p-4 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2" role="group" aria-label="预览缩放与翻页">
          <Button aria-label="缩小预览" onClick={() => onScaleChange(scale - 0.05)} size="sm" variant="secondary">
            <ZoomOut aria-hidden="true" className="size-4" />
          </Button>
          <span className="min-w-14 text-center text-sm font-medium tabular-nums text-text-muted">{Math.round(scale * 100)}%</span>
          <Button aria-label="放大预览" onClick={() => onScaleChange(scale + 0.05)} size="sm" variant="secondary">
            <ZoomIn aria-hidden="true" className="size-4" />
          </Button>
        </div>
      </div>

      <div className="mt-5 overflow-auto rounded-xl border border-border bg-border/30 p-4 sm:p-6">
        <div className="mx-auto" style={{ height: `${scaledHeight}mm`, width: `${scaledWidth}mm` }}>
          <div style={{ transform: `scale(${scale})`, transformOrigin: 'top left' }}>
            <LabelPageCanvas appearance={appearance} maxLines={maxLines} page={page} screenMode />
          </div>
        </div>
      </div>
