...
export function LabelContentPanel() {
  ...
  return (
    <section aria-labelledby="content-heading" className="rounded-2xl border border-border bg-surface-raised p-5 shadow-sm">
      ...
      <label className="mt-5 flex min-h-11 cursor-pointer items-center gap-3 text-sm font-medium text-text"><input checked={appearance.allowWrap} className="size-5 accent-primary" onChange={(event) => setStyle('allowWrap', event.target.checked)} type="checkbox" /><WrapText aria-hidden="true" className="size-4 text-text-muted" />允许姓名和班级换行（最多按当前模板内容容量显示）</label>
      <fieldset className="mt-5 rounded-xl border border-border bg-surface p-4">
        <legend className="px-1 text-sm font-semibold text-text">字段标题</legend>
        <p className="mt-1 text-xs leading-5 text-text-muted">可选择是否在每个标签中显示“班级”和“姓名”标题。</p>
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          <label className="flex min-h-11 cursor-pointer items-center gap-3 rounded-lg px-2 text-sm font-medium text-text hover:bg-surface-muted has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-60"><input checked={appearance.showClassTitle} className="size-5 accent-primary" disabled={!hasClassData} onChange={(event) => setStyle('showClassTitle', event.target.checked)} type="checkbox" />显示“班级”标题</label>
          <label className="flex min-h-11 cursor-pointer items-center gap-3 rounded-lg px-2 text-sm font-medium text-text hover:bg-surface-muted"><input checked={appearance.showNameTitle} className="size-5 accent-primary" onChange={(event) => setStyle('showNameTitle', event.target.checked)} type="checkbox" />显示“姓名”标题</label>
        </div>
        {!hasClassData ? <p className="mt-2 text-xs leading-5 text-text-muted">导入并确认班级列后，可开启班级标题。</p> : null}
      </fieldset>
    </section>
  )
}
