...
export function PrintableCalibrationDocument({ paper }: Readonly<{ paper: PaperSettings }>) {
  ...
  return <div aria-hidden="true" className="print-document calibration-document" data-testid="printable-calibration-document">{markup.map((svg, index) => <div className="print-svg-page label-page" dangerouslySetInnerHTML={{ __html: svg }} key={index} />)}</div>
}
