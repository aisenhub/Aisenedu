...
export function PrintCalibrationPanel({ onPrintCalibration }: Readonly<{ onPrintCalibration: () => void }>) {
  ...
  return (
    <section aria-labelledby="calibration-heading" className="rounded-2xl border border-border bg-surface-raised p-5 shadow-sm">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium text-text">当前偏移</p><p className="text-sm tabular-nums text-text-muted" id="calibration-note">X {Number(layout.offsetXmm).toFixed(1)}mm · Y {Number(layout.offsetYmm).toFixed(1)}mm</p></div><Button onClick={onPrintCalibration} size="sm" variant="secondary"><Crosshair aria-hidden="true" className="size-4" />打印校准页</Button></div>
    </section>
  )
}
