...
- 以后凡是涉及网页页面、组件、设计系统、布局、配色、字体、交互、响应式、图表、可访问性或视觉优化的任务，必须使用已安装的 `ui-ux-pro-max` 技能。
- 新页面、新模块或整体视觉方向：先分析产品类型、用户角色、使用场景和技术栈，再使用 `--design-system` 生成设计方向；Aisenedu 当前使用 React + Tailwind + shadcn/ui，应补充对应的 `--stack` 指导。
- 针对已有组件或单一问题：使用一个明确的 `--domain` 查询，例如 `ux`、`typography`、`color`、`react`、`icons` 或 `chart`，不要把无关问题混在同一次查询中。
- 开始实现前，检查项目已有的设计系统文档和页面覆盖规则；如果后续生成 `design-system/`，以 `MASTER.md` 为全局规范，以页面级文档为局部覆盖。
- 交付 UI 前至少检查可访问性、交互反馈、性能、响应式布局、排版/色彩对比和错误/空状态；不要只验证“能渲染”。
- 技能输出是设计建议，不得覆盖用户明确的产品需求、教育场景、权限规则、品牌约束或现有组件库规范。
