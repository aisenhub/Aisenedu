...
UI 遵循 `components/ui/` 中的 shadcn/ui 按需组件和语义 token；姓名贴页面为浅色工作区，桌面端采用左配置/右预览，窄屏纵向排列。空状态、加载态、错误摘要、字段内联错误、确认对话框、撤销 Toast、响应式、四步工作流和懒加载失败重试均有实现或自动化覆盖。姓名贴的字体、色盘、纯色边框、本地背景图和非敏感本地模板配置详见其 feature 架构文档。
