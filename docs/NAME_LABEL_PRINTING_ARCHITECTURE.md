...
1. 教师可在 3 分钟内从一列姓名得到可打印的 A4 姓名贴。
...
| 未识别姓名列 | 显示列名/示例值选择器；未选择前不启用继续操作。 |
| 空行、重复或过长姓名 | 分组展示数量和具体行号；空行移除，重复项默认保留但醒目提示，超长项提示用户修改或允许换行。 |
...
“姓名”是 MVP 唯一必需且唯一默认打印的学生字段。参考页面中“单人铺满/教师批量/桌牌”与拼音、班级、学号的设计对教育场景有价值，但会改变信息密度和隐私风险，因此按以下顺序扩展：

| 能力 | 阶段 | 交互与安全规则 |
| --- | --- | --- |
| 教师批量姓名贴 | MVP | 当前唯一实现的输出模式；适配标签纸、多页、起始格与校准。 |
...
| 班级、学号 | 第二阶段 | 均为可选字段，默认关闭；采用字段选择器而不是默认采集。学号属于更敏感的学生标识，仍只在浏览器内存处理。 |
...
type LabelLayout = {
  ...
  gapXmm: LengthMm
  gapYmm: LengthMm
  firstLabelIndex: number // 0 起始，表示本次打印从第几个格开始
  offsetXmm: LengthMm
  offsetYmm: LengthMm
}
...
`useLabelPrintingStore` 保存最后一次有效的当前草稿、导入步骤、选中姓名列、预览缩放、打印状态与非持久化的提示状态。原始表单字符串由对应的 feature hook 持有；store 不保存服务端数据，也不负责计算页面位置。

建议的 action：`setNames`、`selectTemplate`、`restoreSelectedTemplateDefaults`、`updatePaper`、`updateLayout`、`updateAppearance`、`setPreviewScale`、`resetDraft`。恢复当前模板默认参数只重置纸张、布局、外观和校准偏移，不清空姓名名单。批量导入仅调用一次 `setNames`，不得逐行触发大量状态更新。

### 6.3 隐私与临时数据

- 姓名数据仅在内存保留，不写入 `localStorage`、`sessionStorage`、URL、分析事件或错误日志。
- 可将“最近使用的非敏感模板设置”另行存入 `localStorage`，键名需版本化，例如 `aisenedu:name-label-settings:v1`；该数据严禁包含姓名、原始文件名或班级信息。
...
| 渠道 | 要求 | 输出 |
| --- | --- | --- |
| 多行粘贴/手动输入 | 每行一个姓名 | 单列记录，行号从 1 开始。 |
| CSV | UTF-8，支持逗号与常见引号转义 | 表格、列标题与行号。 |
| XLSX | 读取第一个工作表；允许用户选择列 | 表格、列标题与行号。 |
...
4. 重复姓名不自动删除：教师可能需要同名学生；在列表中标记重复次数。
5. 仅限制显示安全长度，例如 80 个字符；超过长度需修改后才能打印。
...
- CSV/XLSX 解析、列选择和清洗均应产生可测的受限中间模型；只在用户确认姓名列后，单次调用 `setNames` 写入已清洗名单。
...
absoluteSlot = firstLabelIndex + nameIndex
...
每一个输出页都有 `widthMm`、`heightMm` 和二维 cell 数组。起始格前的 cell 为 `empty`，其余 cell 是 `label` 或末尾空格。跨页后从第一格继续填充。
...
0 <= firstLabelIndex < columns × rows
```
...
所有会影响物理输出的字段（纸张尺寸、标签尺寸、页边距、标签间距、起始格、X/Y 偏移）必须提供就近帮助说明，解释其作用与单位；帮助不能只用无文字图标表达。
...
模板详情中提供“恢复当前模板默认参数”。执行前显示将重置哪些设置（纸张、行列、尺寸、间距、外观、校准偏移），但保留当前姓名名单与起始空白格选择；该操作需要可撤销或确认。

### 9.2 自定义

自定义模板允许改纸张尺寸、页边距、行列、标签宽高与间距。模板名称是非敏感配置，可在用户主动选择“保存设置”时本地保存；首版不提供云端共享。
...
- 单元测试：mm 容纳校验、分页、起始格、偏移、空名单、跨页、Unicode 名称、导入上限与 CSV/XLSX 标准化。
- 组件测试：字段错误与最后有效预览、不可打印状态、导入列选择、清空确认、预览页数、模板恢复后姓名/起始格保留。
- 浏览器测试：粘贴名单 → 配置布局 → 调用打印动作；模拟打印回调，断言姓名不进入 URL、持久化存储或网络请求。浏览器自动化不声称能验证系统打印对话框的最终物理输出。

### 12.2 人工打印检查

在 Chrome 与 Edge 的打印预览中分别检查 A4 纵/横向、多页、起始格、校准页以及 `@page` 尺寸。另在至少一台常用 A4 打印机、100% 缩放、背景开关两种配置下验证：预设模板、部分用过的标签纸、中文姓名、英文长姓名、打印校准与多页输出。记录浏览器、打印机、驱动、纸张型号、缩放设置与实测偏差；未实测的模板必须标记为通用模板。
...
| 内容字段与输出模式 | MVP 只打印批量姓名贴 | 拼音、班级、学号、单人铺满、桌牌和主题模板均须按第 4.5 节逐项完成隐私、排版与打印验证后再扩展。 |
