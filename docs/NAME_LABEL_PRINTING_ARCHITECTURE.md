...
桌面（`>= 1024px`）采用两栏工作台：顶部先显示跨两栏的紧凑四步导航，下方左侧为当前步骤内容，右侧为固定可见的预览与打印操作；页面只保留主文档纵向滚动，不再在左栏建立独立纵向滚动。主操作始终是“打印姓名贴”；次要操作包括导入、清空和校准。
...
| 能力 | 阶段 | 交互与安全规则 |
| --- | --- | --- |
| 教师批量姓名贴 | MVP | 当前唯一实现的输出模式；姓名为必选字段，班级为可选字段；适配标签纸、多页、起始格与校准。 |
