...
XLSX 解析使用 `read-excel-file@9.3.10`（MIT），仅在用户选择 XLSX 文件后动态加载，优先使用兼容入口，失败后再尝试浏览器专用入口；CSV 继续使用 feature 内的受限解析器。浏览器打印使用 `react-to-print@3.3.0`（MIT）。解析器不进入首页首屏 chunk，文件仍受 5MB、10,000 行、100 列和 80 字符限制。
