...
- 导入：多行文本、UTF-8 CSV、XLSX 第一个工作表；表格可映射班级与姓名，标签内可分别显示或隐藏两个字段标题
- 上限：文件 5MB、数据行 10,000、列 100、单个姓名或班级值 80 个 Unicode 字符
- 数据边界：姓名只保留在当前页面内存；仅纸张、排版和外观等安全设置写入白名单 `localStorage`，姓名、导入行、背景 object URL 和原始测量不写入 URL、持久化存储、日志或网络请求
- 打印：优先生成关闭打印缩放的矢量 PDF；系统无衬线中文使用随应用分发、OFL 1.1 授权的 Noto Sans SC Regular 子集，其他无法嵌入的字体会明确提示并可使用兼容模式浏览器打印。通用模板均需用户自行实物校准，不承诺特定厂商标签纸精度

XLSX 解析使用 `read-excel-file@9.3.10`（MIT），仅在用户选择 XLSX 文件后动态加载，优先使用兼容入口，失败后再尝试浏览器专用入口；CSV 继续使用 feature 内的受限解析器。PDF 使用 `pdf-lib@1.17.1`（MIT）与 `@pdf-lib/fontkit@1.1.1`（MIT），浏览器打印使用 `react-to-print@3.3.0`（MIT）。解析器和 PDF 依赖不进入首页首屏 chunk，文件仍受 5MB、10,000 行、100 列和 80 字符限制。

Playwright Chromium 已完成页面加载、空状态、文本/CSV 导入和无横向滚动检查；Edge 专项、浏览器系统打印对话框及真实打印机硬件无法由本轮自动化环境可靠控制，正式使用仍需在目标浏览器/打印机上确认 100% 缩放、关闭“适合页面”、纸张进纸和 X/Y 校准。
